...
void Neumann::Initialization(bool isWaveGeneration)
{
    Parent_Algorithm::Initialization(isWaveGeneration);
    qDebug().noquote()
        << QString("[Neumann] %1^3 grid (%2 voxels), %3 seeds, 6-neighbor (von Neumann)%4")
               .arg(numCubes)
...
#pragma omp critical
    ...
    if (getDone()) {
        qDebug().noquote()
            << QString("[Neumann] Step %1: filled %2/%3 (%4%) | growth complete")
        ...
        qDebug().noquote()
            << QString("[Neumann] Step %1: filled %2/%3 (%4%) | active front: %5 voxels")
                   .arg(IterationNumber, 2)
                   .arg(filled_voxels)
                   .arg(counter_max)
                   .arg(pct, 5, 'f', 1)
...
MATVIZ_REGISTER_ALGORITHM(AlgorithmPlugin{
    "Neumann",
    "Cellular automaton grain growth with 6-neighbor von Neumann neighborhood, "
    "with material and texture selection.",
    /*order=*/ 3,
    standardGrainGrowthSchema(),
    [](const Parameters& p) {
        return std::make_shared<Neumann>(static_cast<short int>(p.getSize()), p.getPoints());
    }
});
