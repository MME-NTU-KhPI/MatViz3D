...
int32_t Voronoi::nearestSeed(int x, int y, int z, double& sum_p) const
{
    ...
    for (int r = 0; r <= maxR; ++r) {
        ...
        if (r >= 2 && bestId != 0 && axisPow((r - 1) * cell) > best)
            break;
            ...
            for (int dy = -r; dy <= r; ++dy) {
                ...
                for (int dz = -r; dz <= r; ++dz) {
                    ...
                    for (int32_t k = m_grid.start[b]; k < m_grid.start[b + 1]; ++k) {
                        ...
                        int ex = std::abs(x - s.x);
                        int ey = std::abs(y - s.y);
                        int ez = std::abs(z - s.z);
                        ...
                        // Compared un-rooted: t -> t^(1/p) is increasing for
                        // p > 0, so this orders exactly like d_p does.
                        const double s_p = m_pow[ex] + m_pow[ey] + m_pow[ez];
                        if (s_p < best) {
                            best   = s_p;
...
void Voronoi::Initialization(bool /*isWaveGeneration*/)
{
    ...
    qDebug().noquote()
        << QString("[Voronoi] %1^3 grid, %2 seeds, Minkowski p = %3%4")
...
void Voronoi::verifyAgainstBruteForce() const
{
    ...
    for (int i = 0; i < nSamples; ++i) {
        ...
        for (const Coordinate& s : seedPoints) {
            int ex = std::abs(x - s.x), ey = std::abs(y - s.y), ez = std::abs(z - s.z);
            if (m_periodic) {
                ex = std::min(ex, N - ex);
                ey = std::min(ey, N - ey);
                ez = std::min(ez, N - ez);
            }
            brute = std::min(brute, m_pow[ex] + m_pow[ey] + m_pow[ez]);
        }
    ...
    for (int x = 0; x < N; ++x)
        for (int y = 0; y < N; ++y)
            for (int z = 0; z < N; ++z) {
                ...
                if (m_pow[ex] + m_pow[ey] + m_pow[ez] > best * (1.0 + 1e-12))
                    ++wrong;
            }
...
MATVIZ_REGISTER_ALGORITHM(AlgorithmPlugin{
    "Voronoi",
    "Voronoi tessellation under a Minkowski L_p metric (p = 1 octahedral, "
    "2 Euclidean, large p cuboidal), with material and texture selection.",
    /*order=*/ 0,
    voronoiSchema(),
    [](const Parameters& p) {
