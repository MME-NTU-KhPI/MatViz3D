...
// Windows headers (pulled in transitively by Qt) define `near` and `far` as
// empty macros for legacy 16-bit pointer qualifiers.  Those names clash with
// the Ctx::near() method below, so we undefine them immediately after all
// system and Qt headers have been included.
#ifdef near
#  undef near
#endif
#ifdef far
