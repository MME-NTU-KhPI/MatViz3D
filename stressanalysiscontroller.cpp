...
void StressAnalysisController::saveSingleShotResult()
{
    ...
    const QString group = saveSingleShotResultToHDF5(filename, m_lastResult, solver, Parameters::seed, m_lastEps);
    if (group.isEmpty()) {
        setError(tr("Failed to save single-shot result to HDF5"));
        return;
    }
