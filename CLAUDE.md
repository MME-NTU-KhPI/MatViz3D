...
The older algorithms — `Moore`, `Neumann`, `Radial`, `DLCA`, `Probability_Algorithm`, `Probability_Circle`, `Probability_Ellipse` — still use the legacy split registration in algorithmfactory.cpp (`registerAlgorithms()` + `registerSchemas()`), which writes into the same registry. They are being migrated to `AlgorithmPlugin` one at a time.
