...
The older algorithms — `DLCA`, `Probability_Algorithm`, `Probability_Circle`, `Probability_Ellipse` — still use the legacy split registration in algorithmfactory.cpp (`registerAlgorithms()` + `registerSchemas()`), which writes into the same registry. `Voronoi`, `Composite`, `Moore`, `Neumann`, and `Radial` are all self-registering `AlgorithmPlugin`s.
